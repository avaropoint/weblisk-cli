...
| **Grants** | **Done, and this row was wrong.** Checked against the specs on 2026-09-09: a second operator's admission works end to end — `weblisk operator init`, then `weblisk operator register --orch <url>` lands them at `status: "pending"`, an admin sees them in `weblisk operators list` (which calls that out, with the command to run), admits them with `weblisk operators approve <name>`, and they collect their own token with `weblisk operator token`. `operators revoke` and `operators role` are implemented too. `architecture/admin.md` defines seven operator endpoints; `pkg/tenant/routes.go` declares seven; they correspond one to one, and `architecture/cli.md` names no verb this repo lacks. An earlier correction to this row claimed an `invite` was still missing — it is not missing from the CLI, it belongs to `patterns/principal-identity`, a three-layer identity/credential/grant model for subjects spanning SEVERAL hubs, which defines no endpoints and which no hub implements. See ROADMAP.md item 1 |
