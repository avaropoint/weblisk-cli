...
func TestRepairIsBoundedAndReportsHonestly(t *testing.T) {
	...
	result, _, err := BuildAndRepair(p, plan, dir, "platform",
		[]GeneratedFile{{Path: "main.go", Content: broken}}, nil, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	if result.OK {
...
func TestAnAssertionNamingNoPlannedFileIsNotRepairedBlindly(t *testing.T) {
	...
	result, _, err := BuildAndRepair(p, plan, dir, "", []GeneratedFile{{Path: "main.go", Content: src}}, nil, checklist, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	if !result.OK {
